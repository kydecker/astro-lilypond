...
% The excerpt as published on lilypond.org references custom notation
% macros from Bača's full score that weren't included in the snippet
% (\fraction, \beam, \square, \triangle, \blackdiamond, \semicircle,
% \harmonic, \fX, \ffX, \sffp). These are stand-in approximations so the
% file compiles; they won't exactly match the original engraving.
fraction = #(define-music-function (music) (ly:music?) #{ $music #})

square = #(define-music-function (note) (ly:music?)
  #{ \tweak NoteHead.style #'cross $note #})
triangle = #(define-music-function (note) (ly:music?)
  #{ \tweak NoteHead.style #'triangle $note #})
blackdiamond = #(define-music-function (note) (ly:music?)
  #{ \tweak NoteHead.style #'harmonic-black $note #})
semicircle = #(define-music-function (note) (ly:music?)
  #{ \tweak NoteHead.style #'slash $note #})
harmonic = #(define-music-function (note) (ly:music?)
  #{ \tweak NoteHead.style #'harmonic $note #})

beam = #(define-music-function (left right note) (number? number? ly:music?)
  #{ $note #})

fX = _\markup { \italic "fx" }
ffX = _\markup { \italic "ffx" }
sffp = _\markup { \dynamic "sfp" }
