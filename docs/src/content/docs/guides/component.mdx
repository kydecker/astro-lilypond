...
When you need the score's page count, header metadata, raw source text, or a downloadable PDF, use the `render` function. Import `render` from `astro-lilypond` and pass it an imported LilyPond score as the first argument.
...
The result of `render` includes a `Score` component (used identically as above), `meta` object, `raw` score text, and optional `pdf`. See the [component reference](/reference/component/#render) for more info.
