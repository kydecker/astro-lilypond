...
`loading` hint forwarded onto every rendered `<img>` (each page in a multi-page group included). Set `loading="lazy"` so off-screen scores in a list don't fetch until scrolled near; for an above-the-fold or LCP score, use `loading="eager"` with [`fetchpriority="high"`](#fetchpriority), or just [`priority`](#priority).
...
A convenience for an above-the-fold or LCP score. When `true`, sets `loading="eager"`, `decoding="sync"`, and `fetchpriority="high"` — the same defaults Astro's `<Image>` derives from its own `priority` prop. Any of [`loading`](#loading)/[`decoding`](#decoding)/[`fetchpriority`](#fetchpriority) you pass explicitly take precedence over these.

```astro title="HeroScore.astro"
