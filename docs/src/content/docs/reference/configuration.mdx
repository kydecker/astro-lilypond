...
Downloads are cached per OS (e.g. `~/.cache/astro-lilypond` on Linux). The cache directory can be overridden with the `ASTRO_LILYPOND_CACHE_DIR` environment variable.
