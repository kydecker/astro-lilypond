...
A `render()` call can always override this with an explicit `crop: true`/`crop: false` — see the [component guide](/guides/component/#cropping).
...
The output format used by [Markdown](/guides/markdown/) fences. Does not affect `.ly`/`.ily` imports or [`lilypondLoader()`](/guides/collections/) entries, which are format-agnostic until a [`render()`](/reference/render/) call picks a format (including `"pdf"`, for a downloadable rendering — see [PDF download](/guides/component/#pdf-download)).
