...
Downloads are cached per OS (e.g. `~/.cache/astro-lilypond` on Linux). The cache directory can be overridden with the `ASTRO_LILYPOND_CACHE_DIR` environment variable. Prebuilt releases are available for Linux (x86_64), macOS (x86_64/arm64), and Windows (x86_64); on other platforms, install LilyPond manually.
